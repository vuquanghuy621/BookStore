    ...
    validationSchema: Yup.object({
      bookId: Yup.string()
        .test("is-use", "Mã sách đã tồn tại!", async function (value) {
          if (!value) return true; // Optional for auto-generation
          try {
      ...
      image: Yup.mixed().required("Không được bỏ trống trường này!")
        ...
        .test("FILE_FORMAT", "File không đúng định dạng!", (value) =>
          !value || (value && ['image/png', 'image/gif', 'image/jpeg', 'image/webp'].includes(value?.type))
        )
    }),
    onSubmit: async () => {
      const { bookId, name, author, genre, publisher, description,
        year, pages, size, price, discount, image, stock } = formik.values;
      ...
      try {
        ...
        setLoading(true)
        const resCloudinary = await axios.post("https://api.cloudinary.com/v1_1/dnj9dpp3f/image/upload", formData)
                ...
                <Col xl={3}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      placeholder="Mã sách (Tự động tạo nếu để trống)"
                ...
                <Col xl={3}>
                  <div className="form-group">
                    ...
                    <input
                      ...
                      name="stock"
                      min="0"
                      className={`form-control ${formik.errors.stock && formik.touched.stock
                        ? "is-invalid"
                        : "is-valid"
                        }`}
